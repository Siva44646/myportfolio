...
// Postgres Connection (Requires POSTGRES_URL)
const pool = new Pool({
...
const initializeDB = async () => {
  try {
    ...
    if (parseInt(projectsRes.rows[0].count) === 0) {
      await pool.query(
        "INSERT INTO projects (title, description, tech, github, demo, color) VALUES ($1, $2, $3, $4, $5, $6)",
        ['KAAM-SETU', 'A platform connecting workers and customers...', 'JavaScript,Node.js,PostgreSQL', '#', 'https://kaam-setu-two.vercel.app/', 'from-cyan-500/20 via-blue-500/10 to-transparent']
      );
      await pool.query(
        "INSERT INTO projects (title, description, tech, github, demo, color) VALUES ($1, $2, $3, $4, $5, $6)",
        ['Education Platform for Disabled Students', 'An inclusive educational platform...', 'HTML,CSS,JavaScript,React.js', '#', '#', 'from-purple-500/20 via-pink-500/10 to-transparent']
      );
    }
    ...
    if (parseInt(contentRes.rows[0].count) === 0) {
      const defaultContent = {
        ...
        skills: [
          ...
          { title: "Database", skills: ["PostgreSQL"], color: "from-purple-400/20 to-pink-500/20", borderColor: "group-hover:border-purple-400/50" },
...
// We only attempt to initialize if we actually have a DB URL
...
app.put('/api/projects/:id', authenticateToken, async (req, res) => {
  ...
  try {
    await pool.query(
      "UPDATE projects SET title = $1, description = $2, tech = $3, github = $4, demo = $5, color = $6 WHERE id = $7",
      [title, description, tech, github, demo, color, req.params.id]
    );
...
app.post('/api/projects', authenticateToken, async (req, res) => {
  ...
  try {
    const result = await pool.query(
      "INSERT INTO projects (title, description, tech, github, demo, color) VALUES ($1, $2, $3, $4, $5, $6) RETURNING id",
      [title, description, tech, github, demo, color]
    );
